...
import { Loader2, Search, Check, Link2, ExternalLink, Sparkles, Eye } from "lucide-react";
      ...
      {created && (
        <div className="mt-6 rounded-2xl border border-[#FF385C]/30 bg-[#FF385C]/[0.04] p-5">
          ...
          <p className="mt-2 text-xs text-[#717171]">
            Copy this link and paste it into your own email or message to {created.guestName || "the guest"}.
          </p>
          <div className="mt-3 flex flex-wrap gap-2">
            <button onClick={() => copy(created.id)} className="inline-flex items-center gap-1.5 rounded-full bg-[#FF385C] px-3.5 py-2 text-sm font-semibold text-white hover:opacity-90">
              {copiedId === created.id ? <Check className="size-4" /> : <Link2 className="size-4" />}
        ...
        <div className="mt-6 space-y-5">
          ...
          {selected.length > 0 && (
            <div className="rounded-2xl border border-[#EBEBEB] bg-white p-5">
              ...
              <div className="mt-4">
                <input className={field} placeholder="Guest first name (personalises the page)" value={guestName} onChange={(e) => setGuestName(e.target.value)} />
              </div>
              <textarea
